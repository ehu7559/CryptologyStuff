...
#   Of course, I did have to type them, but I only had to do it once
...
class AES_CBC_128:
    def encrypt(data: bytes, aes_key: bytes, iv: bytes) -> bytes:
        ...
        while not padded:
            ...
            #which probably doesn't matter for Python, but it's fun :)
...
def gen_block(aes_key: bytes, nonce: int, ctr: int) -> bytes:
    ...
    return AES_ECB_128.encrypt(bytes(block), aes_key)
...
class AES_CTR_128:
    ...
    def decrypt(data: bytes, key: bytes, nonce: int) -> bytes:
        return AES_CTR_128.encrypt(data, key, nonce)
